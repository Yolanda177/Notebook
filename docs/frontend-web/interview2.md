...
**强缓存**：根据响应头的 **Cache-Control** 相对时间 (http/1.1)或**Expires** 绝对时间 (http/1.0)控制；Cache-Control 优先级高于 Expires
  ...
  | no-cache |  不使用强缓存，浏览器向服务器发送请求，验证资源是否还有效 |
...
**HTTPS**（安全超文本传输协议）是一种用于在互联网上安全传输数据的协议。他是 HTTP（超文本传输协议）的安全版本，通过在 HTTP 中加入**安全层SSL/TCL**协议来加密数据包，确保数据传输的安全性和完整性。

HTTPS = HTTP + 加密 + 认证 + 完整性保护

**特点**

- 数据加密：HTTPS 使用 SSL/TCL 协议对数据进行加密，使得传输的数据不易被截获和篡改
- 身份验证：通过 SSL/TCL 证书，HTTPS 可以验证服务器身份，确保用户与真正的服务器通信，而不是一个伪装的服务器
- 数据完整性：HTTPS 确保数据在传输过程没被篡改，维护数据完整性
- 证书管理：HTTPS 需要到证书颁发机构 CA 申请获取 SSL/TCL 证书，证书需要定期更新维护
...
| 类型 | HTTPS | HTTP |
| :--: | :---: |:---: |
| 安全性 | 加入了 SSL/TCL 协议进行数据加密和完整性、身份校验 | 明文传输，可能被篡改和截获 |
| 传输方式 | 在 SSL/TCL 安全层上，SSL/TCL 运行在 TCP 之上 | 直接与 TCP 进行数据传输 |
| 端口 | 默认 443 | 默认 80 |
...
**async**
- async 属性指示浏览器**在脚本下载完成后，尽快执行脚本，无论HTML文档是否已经解析完成**
